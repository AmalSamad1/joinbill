...
import 'package:ziptech/view/purchase/purchase_list_widget.dart';
...
class PurchaseReturnAddWidget extends StatefulWidget {
  final bool isDrawer;
  final bool? isEdit;
  final Map? data;
  final String? id;
  const PurchaseReturnAddWidget(this.isDrawer,
      {Key? key, this.isEdit, this.data, this.id})
      : super(key: key);

  @override
  _PurchaseReturnAddWidgetState createState() =>
      _PurchaseReturnAddWidgetState();
}

class _PurchaseReturnAddWidgetState extends State<PurchaseReturnAddWidget> {
    ...
    billingNoController = !widget.isEdit!
        ? TextEditingController(
            text: salesData.setBill(
                prefix: userData.userData!["billing_settings"]["purchase_return"]
                    ["prefix"],
                number: userData.userData!["billing_settings"]["purchase_return"]
                    ["starting_number"]))
  ...
  Widget build(BuildContext context) {
    ...
    return widget.isDrawer
        ? Scaffold(
              ...
              leading: FlutterFlowIconButton(
                ...
                onPressed: () async {
                  widget.isDrawer
                      ...
                      : context
                          .read<AuthProvider>()
                          .setCurrentPage(PurchaseReturnListWidget(false));
                },
              ),
              title: Text(
                'Add Return',
                                ...
                                salesData.fetchsaleReturn().then((value) =>
                                    widget.isDrawer
                                        ...
                                        : context
                                            .read<AuthProvider>()
                                            .setCurrentPage(
                                                PurchaseReturnListWidget(false)));
                              });
                            },
                            icon: Icon(
                              Icons.delete,
            ...
            body: _purchaseReturnAddMain(context))
        : _purchaseReturnAddMain(context);
  }

  Widget _purchaseReturnAddMain(BuildContext context) {
    final itemData = Provider.of<ItemAddBill>(context);
    final peopleData = Provider.of<PeopleProvider>(context);
    return Form(
      key: _formKey,
