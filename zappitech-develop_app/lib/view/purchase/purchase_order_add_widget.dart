...
class PurchaseOrderAddWidget extends StatefulWidget {
  final bool isDrawer;
  final bool? isEdit;
  final Map ?data;
  final String? id;
  const PurchaseOrderAddWidget(this.isDrawer,
      {Key? key, this.isEdit, this.data, this.id})
      : super(key: key);

  @override
  _PurchaseOrderAddWidgetState createState() => _PurchaseOrderAddWidgetState();
}

class _PurchaseOrderAddWidgetState extends State<PurchaseOrderAddWidget> {
  ...
  void initState() {
    ...
    final salesData =
        Provider.of<PurchaseOederProvider>(context, listen: false);
    ...
    billingNoController = !widget.isEdit!
        ? TextEditingController(
            text: salesData.setBill(
                prefix: userData.userData!["billing_settings"]["purchase_order"]
                    ["prefix"],
                number: userData.userData!["billing_settings"]["purchase_order"]
                    ["starting_number"]))
  ...
  Widget build(BuildContext context) {
    ...
    return widget.isDrawer
        ? Scaffold(
              ...
              leading: FlutterFlowIconButton(
                ...
                onPressed: () async {
                  widget.isDrawer
                      ...
                      : context
                          .read<AuthProvider>()
                          .setCurrentPage(PurchaseOrderListWidget(false));
                },
              ),
              title: Text(
                'Add Order',
                            ...
                            onPressed: () async {
                              await salesData
                                ...
                                id: widget.id,
                                  ...
                                  .then((value) {
                                salesData.fetchSalesOrder().then((value) =>
                                    widget.isDrawer
                                        ? Navigator.pop(context)
                                        : context
                                            .read<AuthProvider>()
              ...
              onPressed: () async {
                if (widget.isEdit!) {
                  await salesData
                      ...
                      .then((value) {
                    salesData.fetchSalesOrder().then((value) => widget.isDrawer
                        ? Navigator.pop(context)
                        : context
                            .read<AuthProvider>()
                            .setCurrentPage(PurchaseOrderListWidget(false)));
            ...
            body: SafeArea(
              child: _purchaseOrderAddMain(context),
            ))
        : _purchaseOrderAddMain(context);
  }

  Widget _purchaseOrderAddMain(BuildContext context) {
    final itemData = Provider.of<ItemAddBill>(context);
    final peopleData = Provider.of<PeopleProvider>(context);
    final userData = Provider.of<AuthProvider>(context, listen: false);
    return Form(
